...
class SVAR_CV(tsbase.TimeSeriesModel):
    def __init__(self, endog, SB, start=None, end=None, freq=None,
        ...
        sigma_hat2=sigma_hat2, p=self.p, TB=TB, SB_character=self.SB_character, crit=crit, y_out=self.y_out, type_=self.type_)
            ...
            self.result = self._identify_volatility(endog, SB=SB, u=self.u, k=self.k, y=self.y , restriction_matrix=restriction_matrix, restrictions=restrictions, sigma_hat1=sigma_hat1,\
                                                sigma_hat2=sigma_hat2, p=self.p, TB=TB, SB_character=self.SB_character, crit=crit, y_out=self.y_out, type_=self.type_)
    ...
    def _identify_volatility(self, x, SB, u, k, y, restriction_matrix, restrictions, sigma_hat1,
        ...
        while np.abs(Exit) > crit and counter < self.max_iter:
            ...
            GLS1_2 = np.sum(np.apply_along_axis(self._gls1, axis=0, arr=Z_t[:, (TB-1):], sig=sig2), axis=1)  # sum of each row
            if x.trend is None:
            ...
            lambda_ = np.ones(k)
            S = np.concatenate((B, lambda_), axis=None)  # S is not changing in the loop in R

            # optimize the likelihood function
            # TODO: Fix error in minimize (sigma_hat1gls or sigma_hat2gls must be false)
            MLE_gls = minimize(fun=self._likelihood, x0=S, args=(self.Tob, TB, sigma_hat1gls, k, sigma_hat2gls, restriction_matrix, restrictions), method='BFGS', tol=1e-6, options={"maxiter": 150})
            ...
            counter += 1
            lambda_hat.append(lambda_hatg)
            B_hat.append(B_hatg)
            ll.append(ll_g)
            GLSE.append(GLS_hat)
            MLE_gls_loop.append(MLE_gls)
